...
@implementation ZXCoreBottomView

UIFont *defalutFont = nil;
+ (void)load {
    defalutFont = ZXFont(15);
}

- (instancetype)init
{
    self = [self initWithBottomY:kScreenHeight];
    if (self) {
    }
    return self;
}
...
- (instancetype)initWithBottomY:(CGFloat)bottomY {
    self = [super init];
    if (self) {
        self.frame = ZXRect(0, 0, kScreenWidth, bottomY);
...
- (void)createUIWithBottomY:(CGFloat)bottomY {
    ...
    contentView.frame = ZXRect(0, self.height, self.width, 0);
...
- (void)reloadUI {
    ...
    self.barLeftLbl.origin = ZXPoint(kBarView_LeftRightMargin, kBarView_TopBottomMargin);
    ...
    view.origin = ZXPoint(0, self.barView.bottom);
...
- (void)show:(BOOL)animated {
    KSELF
    if (!self.superview) [kKeyWindow.rootViewController.view addSubview:self];
    if (animated) {
